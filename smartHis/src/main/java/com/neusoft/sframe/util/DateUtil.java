...
public class DateUtil {
	...
	private static final String DEFAULT_FORMAT_TIMESTAMP = "yyyy-MM-dd HH:mm";//
    ...
    public static String getVorTime(String start,int value){
        ...
        int mode=2;
        if(start.length()==7)mode=1;
        else if(start.length()==10)mode=2;
        else{
        	//我处理不了  你回去吧
        	return "";
        }
        SimpleDateFormat simpleDateFormat = null;
        if(mode==1)simpleDateFormat=new SimpleDateFormat("yyyy/MM");
        if(mode==2)simpleDateFormat=new SimpleDateFormat("yyyy/MM/dd");
        try{
    ...
    /**
     * 判断是否是闰年是则返回366不是返回365
     * @param year
     * @return
     */
	public static int judgeLeapYear(String year) {
