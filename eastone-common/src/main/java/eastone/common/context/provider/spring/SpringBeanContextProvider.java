 ...
 * 的继承关系.为了能够指定parent context,而不需要refresh,必须在构造新的context的时候，通过 构造函数设置parent
 * context。而又不希望提供从外部获得 {@link #context}对象的接口<sub> &lt;注1&gt;</sub>,因此只能在
 * {@link #appendContext(String)}方法内部构造新的Spring Context。<br/>
 * <b>注1:</b>因为开发者使用此接口的时候是通过工厂方法获得的{@link BeanContext}对象,因而无法 正常调用这些接口.</li>
 * </ol>
 * 
...
public class SpringBeanContextProvider implements BeanContext<String> {
   ...
   */
    ...
    public <T> T status(final Object... inputs) {
        ...
        if (key != null) {
            if (key instanceof String) {
                ...
                res = getBean((String) inputs[0], args);
            } else if (key instanceof Class) {
                res = context.getBean((Class<T>) key);
            }
        }
        return res;
    }
    ...
    public BeanContext<String> appendContext(final String newer) {
        ...
        if (StringUtils.startsWithIgnoreCase(path, "classpath:") || (!StringUtils.startsWith(path, "file:") && !new File(path).exists())) {
