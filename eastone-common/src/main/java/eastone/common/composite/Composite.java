/*
 * File:Composite.java
 * Type:eastone.common.composite.Composite
 */
 ...
 * @param <C> 组件接口的基类.
 * <p>
 * 根据使用情况的要求，在API中会提供更为明确的组件基类，比如：在图形交互的应用中，
 * 可能会有所有图形界面组件都继承自<code>GUIComponent</code>接口的设计，这时就应使
 * <code>GUIComponent</code>接口继承{@link Component}接口，作为图形界面组件的公共
 * 基类。此时实现<code>GUIComponent</code>的组合模式时，应当使用
 * <code>GUIComponent</code>作为此参数.
 * </p>
